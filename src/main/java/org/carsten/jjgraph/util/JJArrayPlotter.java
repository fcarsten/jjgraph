...
public class JJArrayPlotter extends JPanel {

	private JJArrayPlotterFrame fenster;
	...
	private java.util.List<JJArrayObject> arrayList;

	Rectangle2D.Double bbox;
	Rectangle2D.Double visiBox;

	public void add(final JJArrayObject l) {
		arrayList.add(l);
		repaint();
	}

	public JJArrayPlotter(final java.util.List<JJArrayObject> ll) {

		arrayList = ll;
		if (ll.isEmpty())
			return;

		JJArrayObject l = ll.get(0);
		Rectangle2D r = l.getBounds2D();
		bbox = new Rectangle2D.Double(r.getX(), r.getY(), Math.max(r.getWidth(), 1), Math.max(r.getHeight(), 1));

		for (final Iterator<JJArrayObject> iter = ll.iterator(); iter.hasNext();) {
			l = iter.next();
			r = l.getBounds2D();
		...
		fenster = new JJArrayPlotterFrame(this);
	...
	public void paintComponent(final Graphics g) {
		...
		for (final Object element : arrayList) {
			final JJArrayObject array = (JJArrayObject) element;
			array.draw(offGraphics, opacity);
		}
