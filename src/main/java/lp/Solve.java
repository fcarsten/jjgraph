...
class Util {
	final static double round(final double val, final double eps) {
...
class Ref {
	...
	public Ref(final double v) {
		value = v;
	}
}

public class Solve implements lp.LpConstants {
	/* Globals */
	...
	void inc_mat_space(final LpRec lp, final int maxextra) {
		if (lp.non_zeros + maxextra > lp.mat_alloc) {
			int i;
			lp.mat_alloc = lp.non_zeros + maxextra;
			final MatRec[] old_mat = lp.mat;
			...
			final int[] old_col_no = lp.col_no;
	...
	void inc_row_space(final LpRec lp) {
		if (lp.rows > lp.rows_alloc) {
			...
			if (lp.names_used != FALSE) {
				final String[] str_ptr = lp.row_name;
	...
	void inc_col_space(final LpRec lp) {
		if (lp.columns >= lp.columns_alloc) {
			...
			if (lp.names_used != FALSE) {
				final String[] str_ptr = lp.col_name;
	...
	public void set_mat(final LpRec lp, final int Row, final int Column, double Value) {
		int elmnr, lastelm, i;
	...
	public void set_obj_fn(final LpRec lp, final double[] row) {
		int i;
		for (i = 1; i <= lp.columns; i++)
			set_mat(lp, 0, i, row[i]);
	}

	public void str_set_obj_fn(final LpRec lp, final String row) {
		double[] arow;
		arow = new double[lp.columns + 1];
		int i = 0;
		final StringTokenizer stk = new StringTokenizer(row);
	...
	public void add_constraint(final LpRec lp, final double[] row, final short constr_type, final double rh) {
		MatRec[] newmat;
		int i, j;
		int elmnr;
		int stcol;
	...
	public void str_add_constraint(final LpRec lp, final String row_string, final short constr_type, final double rh) {
		int i = 0;
		double[] aRow;
		aRow = new double[lp.columns + 1];

		final StringTokenizer stk = new StringTokenizer(row_string);
		while (stk.hasMoreTokens() && i < lp.columns) {
			i++;
			aRow[i] = Double.valueOf(stk.nextToken()).doubleValue();
		}
	...
	public void del_constraint(final LpRec lp, final int del_row) {
		int i, j;
		int elmnr;
		int startcol;
	...
	public void add_lag_con(final LpRec lp, final double[] row, final short con_type, final double rhs) {
		int i;
		double sign = 0;
		...
		if (lp.nr_lagrange == 1) {
			...
			final double[][] db2_ptr = lp.lag_row;
			lp.lag_row = new double[lp.nr_lagrange][];
			...
			final short[] short_ptr = lp.lag_con_type;
			lp.lag_con_type = new short[lp.nr_lagrange];
		...
		lp.lag_con_type[lp.nr_lagrange - 1] = (con_type == EQ) ? (short) 1 : (short) 0;
	}

	public void str_add_lag_con(final LpRec lp, final String row, final short con_type, final double rhs) {
		int i = 0;
		double[] a_row;
	...
	public void add_column(final LpRec lp, final double[] column) {
		int i, elmnr;
	...
	public void str_add_column(final LpRec lp, final String col_string) {
		int i = 0;
		double[] aCol;
		aCol = new double[lp.rows + 1];

		final StringTokenizer stk = new StringTokenizer(col_string);
	...
	public void del_column(final LpRec lp, final int column) {
	...
	public void set_upbo(final LpRec lp, final int column, double value) {
		if (column > lp.columns || column < 1)
			System.err.print("Column out of range");
		if (lp.scaling_used != FALSE)
	...
	public void set_lowbo(final LpRec lp, final int column, double value) {
		if (column > lp.columns || column < 1)
			System.err.print("Column out of range");
		if (lp.scaling_used != FALSE)
			value /= lp.scale[lp.rows + column];
	...
	public void set_int(final LpRec lp, final int column, final short must_be_int) {
		if (column > lp.columns || column < 1)
			System.err.print("Column out of range");
	...
	public void set_rh(final LpRec lp, final int row, final double value) {
		if (row > lp.rows || row < 0)
			System.err.print("Row out of Range");
	...
	public void set_rh_vec(final LpRec lp, final double[] rh) {
		int i;
		if (lp.scaling_used != FALSE)
			for (i = 1; i <= lp.rows; i++)
				if (lp.ch_sign[i] != FALSE)
	...
	public void str_set_rh_vec(final LpRec lp, final String rh_string) {
		int i = 0;
		double[] newrh;
		newrh = new double[lp.rows + 1];

		final StringTokenizer stk = new StringTokenizer(rh_string);
		while (stk.hasMoreTokens() && i < lp.rows) {
			i++;
	...
	public void set_maxim(final LpRec lp) {
		int i;
	...
	public void set_minim(final LpRec lp) {
		int i;
		if (lp.maximise == TRUE) {
			for (i = 0; i < lp.non_zeros; i++)
				if (lp.mat[i].row_nr == 0)
	...
	public void set_constr_type(final LpRec lp, final int row, final short con_type) {
		int i;
	...
	public double mat_elm(final LpRec lp, final int row, final int column) {
		double value;
		int elmnr;
	...
	public void get_row(final LpRec lp, final int row_nr, final double[] row) {
		int i, j;
	...
	public void get_column(final LpRec lp, final int col_nr, final double[] column) {
		int i;
	...
	public void get_reduced_costs(final LpRec lp, final double[] rc) {
		int varnr, i, j;
		double f;
	...
	short is_feasible(final LpRec lp, final double[] values) {
		int i, elmnr;
		double[] this_rhs;
		double dist;
	...
	short column_in_lp(final LpRec lp, final double[] testcolumn) {
		int i, j;
		short ident;
		double value;
		...
		if (lp.scaling_used != FALSE)
			for (i = 1; i <= lp.columns; i++) {
				...
				while (ident != FALSE && (j < lp.col_end[i])) {
					...
					if (Math.abs(
							value) > 0.001) /* should be some epsilon? MB */
						ident = FALSE;
					j++;
				}
				if (ident != FALSE)
	...
	public void print_lp(final LpRec lp) {
		int i, j;
		double[] fatmat;
		...
		System.err.println("problem name: " + lp.lp_name);
		System.err.print("          ");
		for (j = 1; j <= lp.columns; j++)
			if (lp.names_used != FALSE)
				System.err.print(lp.col_name[j]);
			else
				System.err.print("Var[" + j + "] ");
		if (lp.maximise != FALSE) {
			System.err.print("\nMaximise  ");
			for (j = 0; j < lp.columns; j++)
				System.err.print(" " + (-fatmat[j * (lp.rows + 1)]) + " ");
		} else {
			System.err.print("\nMinimize  ");
			for (j = 0; j < lp.columns; j++)
				System.err.print(" " + fatmat[j * (lp.rows + 1)] + " ");
		...
		for (i = 1; i <= lp.rows; i++) {
			if (lp.names_used != FALSE)
				System.err.print(lp.row_name[i]);
			else
				System.err.print("Row[" + i + "]  ");
			for (j = 0; j < lp.columns; j++)
				if (lp.ch_sign[i] != FALSE && fatmat[j * (lp.rows + 1) + i] != 0)
					System.err.print((-fatmat[j * (lp.rows + 1) + i]) + " ");
				else
					System.err.print(fatmat[j * (lp.rows + 1) + i] + " ");
			if (lp.orig_upbo[i] == lp.infinite)
				...
				System.err.print(" = ");
			if (lp.ch_sign[i] != FALSE)
				System.err.println(-lp.orig_rh[i]);
			else
				System.err.println(lp.orig_rh[i]);
		}
		...
		for (i = 1; i <= lp.columns; i++)
			if (lp.orig_upbo[lp.rows + i] == lp.infinite)
				...
				System.err.print(lp.orig_upbo[lp.rows + i] + " ");
		System.err.print("\nlowbo     ");
		for (i = 1; i <= lp.columns; i++)
			System.err.print(lp.orig_lowbo[lp.rows + i] + " ");
		System.err.print("\n");
		for (i = 0; i < lp.nr_lagrange; i++) {
			System.err.print("lag[" + i + "]  ");
			for (j = 1; j <= lp.columns; j++)
				System.err.print(lp.lag_row[i][j] + " ");
			if (lp.orig_upbo[i] == lp.infinite)
				if (lp.lag_con_type[i] == GE)
					...
					System.err.print(" = ");
			System.err.println(lp.lag_rhs[i]);
		}
	}

	public void set_row_name(final LpRec lp, final int row, final String new_name) {
		int i;
	...
	public void set_col_name(final LpRec lp, final int column, final String new_name) {
		int i;
	...
	private double minmax_to_scale(final double min, final double max) {
		double scale;
	...
	public void unscale_columns(final LpRec lp) {
		int i, j;
	...
	public void unscale(final LpRec lp) {
		int i, j;
	...
	public void auto_scale(final LpRec lp) {
	...
	public void reset_basis(final LpRec lp) {
		lp.basis_valid = FALSE;
	}

	public double best_solution(final LpRec lp, final int i) {
		return lp.best_solution[lp.rows + i];
	}

	public double obj_fn_value(final LpRec lp) {
		return lp.best_solution[0];
	}

	public void print_solution(final LpRec lp) {
		int i;

		System.err.println("Value of objective function: " + lp.best_solution[0]);

		/* print normal variables */
		for (i = 1; i <= lp.columns; i++)
			if (lp.names_used != FALSE)
				System.err.println(lp.col_name[i] + " " + lp.best_solution[lp.rows + i]);
			else
				System.err.println("Var [" + i + "]  " + lp.best_solution[lp.rows + i]);
		...
		if (lp.verbose != FALSE) {
			...
			for (i = 1; i <= lp.rows; i++)
				if (lp.names_used != FALSE)
					System.err.println(lp.row_name[i] + " " + lp.best_solution[i]);
				else
					System.err.println("Row [" + i + "]  " + " " + lp.best_solution[i]);
		}

		if ((lp.verbose != FALSE || lp.print_duals != FALSE)) {
			if (lp.max_level != 1)
				System.err.print("These are the duals from the node that gave the optimal solution.\n");
			else
				System.err.print("\nDual values:\n");
			for (i = 1; i <= lp.rows; i++)
				if (lp.names_used != FALSE)
					System.err.println(lp.row_name[i] + " " + lp.duals[i]);
				else
					System.err.println("Row [" + i + "]  " + lp.duals[i]);
		}
	} /* Printsolution */

	public void write_LP(final LpRec lp, final PrintStream output) {
		int i, j;
		double[] row;
	 ...
	 *
	 *
	 * MALLOC(column, lp.rows+1, double); marker=0; output.print(
	 * "NAME          %s\n", lp.lp_name); output.print( "ROWS\n"); for(i = 0; i
	 * <= lp.rows; i++) { if(i==0) output.print( " N  "); else
	 * if(lp.orig_upbo[i]==lp.infinite) if(lp.ch_sign[i]) output.print( " G  ");
	 * else output.print( " L  "); else output.print( " E  "); if (lp.names_used
	 * != FALSE) output.print( "%s\n", lp.row_name[i]); else output.print(
	 * "r_%d\n", i); }
	 *
	 ...
	 * get_column(lp, i, column); j=0; if(lp.maximise) { if(column[j] != 0) { if
	 * (lp.names_used != FALSE) output.print( "    %-8s  %-8s  %g\n",
	 * lp.col_name[i], lp.row_name[j], -column[j]); else output.print(
	 * "    var_%-4d  r_%-6d  %g\n", i, j, -column[j]); } } else { if(column[j]
	 * != 0) { if (lp.names_used != FALSE) output.print( "    %-8s  %-8s  %g\n",
	 * lp.col_name[i], lp.row_name[j], column[j]); else output.print(
	 * "    var_%-4d  r_%-6d  %g\n", i, j, column[j]); } } for(j=1; j <=
	 * lp.rows; j++) if(column[j] != 0) { if (lp.names_used != FALSE)
	 * output.print( "    %-8s  %-8s  %g\n", lp.col_name[i], lp.row_name[j],
	 * column[j]); else output.print( "    var_%-4d  r_%-6d  %g\n", i, j,
	 * column[j]); } } if((marker % 2) ==1) { output.print(
	 * "    MARK%04d  'MARKER'                 'INTEND'\n", marker); marker++; }
	 *
	 * output.print( "RHS\n"); for(i = 1; i <= lp.rows; i++) { if(lp.ch_sign[i])
	 * { if (lp.names_used != FALSE) output.print( "    RHS       %-8s  %g\n",
	 * lp.row_name[i], (double)-lp.orig_rh[i]); else output.print(
	 * "    RHS       r_%-6d  %g\n", i, (double)-lp.orig_rh[i]); } else { if
	 * (lp.names_used != FALSE) output.print( "    RHS       %-8s  %g\n",
	 * lp.row_name[i], (double)lp.orig_rh[i]); else output.print(
	 ...
	 * (double)lp.orig_upbo[i]); if(lp.orig_lowbo[i] != 0) output.print(
	 * " LO BND       %-8s  %g\n", lp.col_name[i- lp.rows],
	 * (double)lp.orig_lowbo[i]); } else for(i = lp.rows + 1; i <= lp.sum; i++)
	 * { if(lp.orig_upbo[i] < lp.infinite) output.print(
	 * " UP BND       var_%-4d  %g\n", i - lp.rows, (double)lp.orig_upbo[i]);
	 * if(lp.orig_lowbo[i] != 0) output.print( " LO BND       var_%-4d  %g\n", i
	 * - lp.rows, (double)lp.orig_lowbo[i]); } output.print( "ENDATA\n");
	 * free(column); }
	 */

	public void print_duals(final LpRec lp) {
		int i;
		for (i = 1; i <= lp.rows; i++)
			if (lp.names_used != FALSE)
				System.err.println(lp.row_name[i] + " [" + i + "] " + lp.duals[i]);
			else
				System.err.println("Dual       [" + i + "] " + lp.duals[i]);
	}

	public void print_scales(final LpRec lp) {
		int i;
		if (lp.scaling_used != FALSE) {
			for (i = 0; i <= lp.rows; i++)
				System.err.println("Row[" + i + "]    scaled at " + lp.scale[i]);
			for (i = 1; i <= lp.columns; i++)
				System.err.println("Column[" + i + "] scaled at " + lp.scale[lp.rows + i]);
		}
	}
	...
	public void debug_print_solution() {
		...
		if (Lp.debug != FALSE)
			for (i = Rows + 1; i <= Sum; i++) {
				print_indent();
				if (Lp.names_used != FALSE)
					System.err.println(Lp.col_name[i - Rows] + " " + Solution[i]);
				else
					System.err.println("Var[" + (i - Rows) + "]   " + Solution[i]);
			}
	} /* debug_print_solution */

	public void debug_print_bounds(final double[] upbo, final double[] lowbo) {
		int i;
		...
		if (Lp.debug != FALSE)
			for (i = Rows + 1; i <= Sum; i++) {
				if (lowbo[i] != 0) {
					print_indent();
					if (Lp.names_used != FALSE)
						System.err.println(Lp.col_name[i - Rows] + " > " + lowbo[i]);
					else
						System.err.print("Var[" + (i - Rows) + "]  > " + lowbo[i]);
				}
				if (upbo[i] != Infinite) {
					print_indent();
					if (Lp.names_used != FALSE)
						System.err.println(Lp.col_name[i - Rows] + " < " + upbo[i]);
					else
						System.err.println("Var[" + (i - Rows) + "]  < " + upbo[i]);
				}
			}
	} /* debug_print_bounds */

	public void debug_print(final String format) {
		if (Lp.debug != FALSE) {
			print_indent();
	...
	void set_globals(final LpRec lp) {
		if (Lp != null)
			Lp.active = FALSE;
		Lp = lp;
		Rows = lp.rows;
	...
	private void ftran(final int start, final int end, final double[] pcol) {
		int i, j, k, r;
		double theta;
		...
		for (i = start; i <= end; i++) {
			...
			if (theta != 0)
				for (j = Eta_col_end[i - 1]; j < k; j++)
					pcol[Eta_row_nr[j]] += theta
							* Eta_value[j]; /* cpu expensive line */
			pcol[r] *= Eta_value[k];
		}
	...
	private void btran(final double[] row) {
		int i, j, k;
		double f;
	...
	static short Isvalid(final LpRec lp) {
	...
	private void setpivcol(final short lower, final int varin, final double[] pcol) {
		int i, colnr;
		double f;
	...
	private void minoriteration(final int colnr, final int row_nr) {
	...
	private void rhsmincol(final double theta, final int row_nr, final int varin) {
		int i, j, k, varout;
		double f;

		if (row_nr > Rows + 1) {
			System.err.print("Error: rhsmincol called with row_nr: " + row_nr + ", rows: " + Rows + "\n");
	...
	void invert() {
		...
		if (Lp.print_at_invert != FALSE)
			System.err.print("Start Invert iter " + Lp.iter + " eta_size " + Eta_size + " rhs[0] " + (-Rhs[0]) + " \n");
		...
		if (Lp.print_at_invert != FALSE)
			System.err.print("End Invert                eta_size " + Eta_size + " rhs[0] " + (-Rhs[0]) + "\n");

		JustInverted = TRUE;
		DoInvert = FALSE;
	} /* invert */

	private short colprim(final Ref colnr, final short minit, final double[] drow) {
		int varnr, i, j;
		double f, dpiv;
		...
		/*
		 * System.err.println("dpiv = " + dpiv);
		 * System.err.println("drow[] at colprim"); for(i = 1; i <= Sum; i++) //
		 * DEBUG { System.err.print(drow[i] + " "); } System.err.println();
		 */
		for (i = 1; i <= Sum; i++)
			if (Basis[i] == FALSE)
				if (Upbo[i] > 0) {
	...
	private short rowprim(final int colnr, final Ref row_nr, final Ref theta, final double[] pcol) {
		int i;
		double f = 0, quot;
		...
		if (theta.value < 0) {
			...
			System.err.println("pcol[" + row_nr.value + "] = " + f + ", rhs[" + row_nr.value + "] = "
					+ Rhs[(int) row_nr.value] + " , upbo = " + Upbo[Bas[(int) row_nr.value]]);
		}
		if (row_nr.value == 0) {
		...
		if (row_nr.value > 0)
			Doiter = TRUE;
		if (Lp.trace != FALSE)
			System.err.println("row_prim:" + row_nr.value + ", pivot element:" + pcol[(int) row_nr.value]);

		return ((row_nr.value > 0) ? (short) 1 : (short) 0);
	} /* rowprim */

	private short rowdual(final Ref row_nr) {
		int i;
		double f, g, minrhs;
		short artifs;
		...
		if (Lp.trace != FALSE) {
			if (row_nr.value > 0) {
				System.err.println(
						"row_dual:" + row_nr.value + ", rhs of selected row:           " + Rhs[(int) row_nr.value]);
				if (Upbo[Bas[(int) row_nr.value]] < Infinite)
					System.err.println(
							"               upper bound of basis variable:    " + Upbo[Bas[(int) row_nr.value]]);
			} else
				System.err.print("row_dual: no infeasibilities found\n");
		}

		return (row_nr.value > 0 ? (short) 1 : (short) 0);
	} /* rowdual */

	private short coldual(final int row_nr, final Ref colnr, final short minit, final double[] prow,
			final double[] drow) {
		int i, j, r, varnr;
		...
		if (Lp.trace != FALSE)
			System.err.println("col_dual:" + colnr.value + ", pivot element:  " + prow[(int) colnr.value]);
	...
	private void iteration(final int row_nr, final int varin, final Ref theta, final double up, final Ref minit,
			final Ref low, final short primal, final double[] pcol) {
		int i, k, varout;
		double f;
		double pivot;
		...
		if (Lp.trace != FALSE) {
			System.err.print("Theta = " + theta.value + " ");
			if (minit.value != 0) {
				if (Lower[varin] == FALSE)
					System.err.print("Iteration:" + Lp.iter + ", variable" + varin
							+ " changed from 0 to its upper bound of " + Upbo[varin] + "\n");
				else
					System.err.print("Iteration:" + Lp.iter + ", variable" + varin + " changed its upper bound of "
							+ Upbo[varin] + " to 0\n");
			} else
				System.err.print(
						"Iteration:" + Lp.iter + ", variable" + varin + " entered basis at:" + Rhs[row_nr] + "\n");
			if (primal == 0) {
				f = 0;
				for (i = 1; i <= Rows; i++)
					if (Rhs[i] < 0)
						...
						f += Rhs[i] - Upbo[Bas[i]];
				System.err.println("feasibility gap of this basis:" + f);
			} else
				System.err.println("objective function value of this feasible basis: " + Rhs[0]);
		}
	} /* iteration */
	...
	private int solvelp() {
		...
		if (Lp.trace != FALSE) {
			if (primal != FALSE)
				System.err.print("Start at feasible basis\n");
			else
				System.err.print("Start at infeasible basis\n");
		}
		if (primal == FALSE) {
			drow[0] = 1;
			for (i = 1; i <= Rows; i++)
			...
			Extrad = 0;
		if (Lp.trace != FALSE)
			System.err.println("Extrad = " + Extrad);
		minit = FALSE;
		...
		while (Status == RUNNING) {
					...
					if (flag != FALSE) {
						...
						if (Pcol[row_nr] == 0) {
							...
							if (JustInverted == FALSE) {
								System.err.println("Reinverting Eta");
								DoInvert = TRUE;
							} else {
								System.err.println("Can't reinvert, failure");
								Status = FAILURE;
							}
						} else {
							condensecol(row_nr, Pcol);
			...
			if (DoInvert != FALSE) {
				if (Lp.print_at_invert != FALSE)
					System.err.println("Inverting: Primal = " + primal);
				invert();
			}
		}
	...
	private short is_int(final double value) {
		double tmp;
	...
	private void construct_solution(final double[] sol) {
		int i, j, basi;
		double f;
			...
			for (j = 1; j <= Columns; j++) {
				...
				if (f != 0)
					for (i = Col_end[j - 1]; i < Col_end[j]; i++)
						sol[Mat[i].row_nr] += (f / Lp.scale[Rows + j]) * (Mat[i].value / Lp.scale[Mat[i].row_nr]);
			}
	...
	private int milpsolve(final double[] upbo, final double[] lowbo, final short[] sbasis, final short[] slower,
			final int[] sbas) {
		...
		if (failure != OPTIMAL)
			debug_print("this problem has no solution, it is " + ((failure == UNBOUNDED) ? "unbounded" : "infeasible"));

		if (failure == INFEASIBLE && Lp.verbose != FALSE)
			System.err.print("level" + Level + " INF\n");
		...
		if (failure == OPTIMAL) /* there is a solution */
		{
			...
			if (is_worse != FALSE) {
				if (Lp.verbose != FALSE)
					System.err
							.println("level" + Level + " OPT NOB value " + Solution[0] + " bound " + Best_solution[0]);
			...
			if (Lp.bb_rule == FIRST_NI) {
				...
				while (i <= Sum && notint == 0) {
					if (Must_be_int[i] != FALSE && is_int(Solution[i]) == FALSE)
						...
						{
							System.err.println("Warning: integer var " + (i - Rows) + " is already fixed at " + lowbo[i]
			...
			if (Lp.verbose == TRUE)
				if (notint != FALSE)
					System.err.println("level " + Level + " OPT     value " + Solution[0]);
				else
					System.err.println("level " + Level + " OPT INT value " + Solution[0]);
			...
			if (notint != FALSE) /* there is at least one value not yet int */
			{
				...
				if (Lp.names_used != FALSE)
					debug_print("not enough ints. Selecting var " + Lp.col_name[notint - Rows] + ", val: "
							+ Solution[notint]);
				else
					debug_print("not enough ints. Selecting Var [" + notint + "], val: " + Solution[notint]);
				...
				if (Floor_first != FALSE) {
					...
					if (new_bound < lowbo[notint]) {
						debug_print("New upper bound value " + new_bound + " conflicts with old lower bound "
								+ lowbo[notint] + "\n");
						resone = MILP_FAIL;
					} else /* bound feasible */
					{
						...
						Lp.eta_valid = FALSE;
						resone = milpsolve(new_upbo, lowbo, new_basis, new_lower, new_bas);
						Lp.eta_valid = FALSE;
					}
					new_bound += 1;
					if (new_bound > upbo[notint]) {
						debug_print("New lower bound value " + new_bound + " conflicts with old upper bound "
								+ upbo[notint] + "\n");
						restwo = MILP_FAIL;
					} else /* bound feasible */
					{
						...
						Lp.eta_valid = FALSE;
						restwo = milpsolve(upbo, new_lowbo, new_basis, new_lower, new_bas);
						Lp.eta_valid = FALSE;
					}
				} else /* take ceiling first */
				{
					...
					} else /* bound feasible */
					{
						...
						debug_print_bounds(upbo, new_lowbo);
						Lp.eta_valid = FALSE;
						resone = milpsolve(upbo, new_lowbo, new_basis, new_lower, new_bas);
						Lp.eta_valid = FALSE;
					}
					new_bound -= 1;
					if (new_bound < lowbo[notint]) {
					...
					} else /* bound feasible */
					{
						...
						debug_print_bounds(new_upbo, lowbo);
						Lp.eta_valid = FALSE;
						restwo = milpsolve(new_upbo, lowbo, new_basis, new_lower, new_bas);
						Lp.eta_valid = FALSE;
					}
				}
			...
			} else /* all required values are int */
			{
				...
				if (is_worse == FALSE) /* Current solution better */
				{
					if (Lp.debug != FALSE || (Lp.verbose != FALSE && Lp.print_sol == FALSE))
						System.err.print(
								"*** new best solution: old: " + Best_solution[0] + ", new: " + Solution[0] + " ***\n");
	...
	public int solve(final LpRec lp) {
		int result, i;
	...
	public int lag_solve(final LpRec lp, final double start_bound, final int num_iter, final short verbose) {
		...
		while (status == RUNNING) {
			...
			for (i = 1; i <= lp.columns; i++) {
				...
				/* fSystem.err.print(stderr, "%f ", ModObj[i]); */
			}
			rhsmod = 0;
			for (i = 0; i < lp.nr_lagrange; i++)
				if (lp.maximise != FALSE)
			...
			if (verbose != FALSE) {
				System.err.println(
						"Zub: " + Zub + " Zlb: " + Zlb + " Step: " + Step + " pie: " + pie + " Feas " + OrigFeas);
				for (i = 0; i < lp.nr_lagrange; i++)
					System.err.println(i + " SubGrad " + SubGrad[i] + " lambda " + lp.lambda[i]);
			}
			...
			if (verbose != FALSE)
				System.err.println("result: " + result + "  same basis: " + same_basis);

			if (result == UNBOUNDED) {
				for (i = 1; i <= lp.columns; i++)
					System.err.print(ModObj[i] + " ");
				System.exit(FAIL);
			}
			...
			if (OrigFeas != FALSE) {
				...
				if ((lp.maximise != FALSE) && (Ztmp > Zlb)) {
					...
					BestFeasSol[0] = Zlb;
					if (verbose != FALSE)
						System.err.println("Best feasible solution: " + Zlb);
				} else if (Ztmp < Zub) {
					Zub = Ztmp;
					for (i = 1; i <= lp.sum; i++)
						BestFeasSol[i] = lp.best_solution[i];
					BestFeasSol[0] = Zub;
					if (verbose != FALSE)
						System.err.println("Best feasible solution: " + Zub);
				}
			}
