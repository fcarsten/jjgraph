...
public class LpRec implements LpConstants {
	...
	short names_used; /*
						 * Flag to indecate if names for rows and columns are
						 * used
						 */
	...
	int[] col_end; /*
					 * columns_alloc+1 :Cend[i] is the index of the first
					 * element after column i. column[i] is stored in elements
					 * col_end[i-1] to col_end[i]-1
					 */
	int[] col_no; /*
					 * mat_alloc :From Row 1 on, col_no contains the column nr.
					 * of the nonzero elements, row by row
					 */
	short row_end_valid; /* true if row_end & col_no are valid */
	int[] row_end; /*
					 * rows_alloc+1 :row_end[i] is the index of the first
					 * element in Colno after row i
					 */
	double[] orig_rh; /*
						 * rows_alloc+1 :The RHS after scaling & sign changing,
						 * but before `Bound transformation'
						 */
	double[] rh; /*
					 * rows_alloc+1 :As orig_rh, but after Bound transformation
					 */
	double[] rhs; /*
					 * rows_alloc+1 :The RHS of the curent simplex tableau
					 */
	short[] must_be_int; /*
							 * sum_alloc+1 :TRUE if variable must be Integer
							 */
	...
	short[] basis; /*
					 * sum_alloc+1 : basis[i] is TRUE if the column is in the
					 * basis
					 */
	short[] lower; /*
					 * "       " :TRUE is the variable is at its lower bound (or
					 * in the basis), it is FALSE if the variable is at its
					 * upper bound
					 */
	...
	int max_num_inv; /*
						 * ## The number of double pivots between reinvertions
						 */
	double[] eta_value; /*
						 * eta_alloc :The Structure containing the values of Eta
						 */
	int[] eta_row_nr; /*
						 * "     " :The Structure containing the Row indexes of
						 * Eta
						 */
	int[] eta_col_end; /*
						 * rows_alloc + MaxNumInv : eta_col_end[i] is the start
						 * index of the next Eta column
						 */

	short bb_rule; /* what rule for selecting B&B variables */

	short break_at_int; /*
						 * TRUE if stop at first integer better than break_value
						 */
	double break_value;

	double obj_bound; /*
						 * ## Objective function bound for speedup of B&B
						 */
	int iter; /*
				 * The number of iterations in the simplex solver (LP)
				 */
	...
	double[] solution; /*
						 * sum_alloc+1 :The Solution of the last LP, 0 = The
						 * Optimal Value, 1..rows The Slacks, rows+1..sum The
						 * Variables
						 */
	public double[] best_solution; /* "       " :The Best 'Integer' Solution */
	double[] duals; /*
					 * rows_alloc+1 :The dual variables of the last LP
					 */

	short maximise; /*
					 * TRUE if the goal is to maximise the objective function
					 */
	short floor_first; /* TRUE if B&B does floor bound first */
	short[] ch_sign; /*
						 * rows_alloc+1 :TRUE if the Row in the matrix has
						 * changed sign (a`x > b, x>=0) is translated to s +
						 * -a`x = -b with x>=0, s>=0)
						 */

	short scaling_used; /* TRUE if scaling is used */
	short columns_scaled; /*
							 * TRUE is the columns are scaled too, Only use if
							 * all variables are non-integer
							 */
	double[] scale; /*
					 * sum_alloc+1 :0..Rows the scaling of the Rows, Rows+1..Sum
					 * the scaling of the columns
					 */

	int nr_lagrange; /* Nr. of Langrangian relaxation constraints */
	double[][] lag_row; /*
						 * NumLagrange, columns+1:Pointer to pointer of rows
						 */
	...
	public LpRec(final int nrows, final int ncolumns) {
		int i, nsum;
	...
	//
	public double getBestSolution(final int i) {
